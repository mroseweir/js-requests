...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/tacos').then( (res) => {
...
// CODE HERE
const queryTest = () => {
    axios.get('http://localhost:3000/query-test/?test=tacos').then( (res) => {
        console.log(res.data)
    })
}
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        Got a response stating that I had sent in an empty query

    2: Send more than 1 query on the URL -- what happened? 
        Got a response stating that I had sent in more than 1 query
